...
class CrudMenuController extends Controller
{
    ...
    public function post(Request $request)
    {
        try {
            ...
            if ($request->parent_menu_id != null) {
                ...
                $menu_id        = 0;
                $no_urut  = $request->serial_sub_menu;
                $icon           = '';

                if ($request->serial_sub_menu != $request->max_serial_sub_menu) {
                    DB::table('menus')
                        ->where('parent_menu_id', $request->parent_menu_id)
                        ->where('no_urut', $request->serial_sub_menu)
                        ->update(['no_urut' => $request->max_serial_sub_menu]);
                }
            } else {
                //jika menu
                ...
                $no_urut  = $request->serial_menu;
                $icon           = $request->icon;

                if ($request->serial_menu != $request->max_serial_menu) {
                    DB::table('menus')
                        ->where('parent_menu_id', 0)
                        ->where('no_urut', $request->serial_menu)
                        ->update(['no_urut' => $request->max_serial_menu]);
                }
            }
    ...
    {
        $max        = DB::table('menus')->where('parent_menu_id', $parent_id)->orderBy('no_urut', 'desc')->first();
        $max_serial = $max ? $max->no_urut + 1 : 1;
        ...
        try {
            ...
            if ($cek['access'] == 0) {
                ...
                if ($select->parent_menu_id != 0) {
                    $max                         = DB::table('menus')->where('parent_menu_id', $select->parent_menu_id)->orderBy('no_urut', 'desc')->first();
                    $data['max_serial_sub_menu'] = $max->no_urut ?? 1;
                } else {
            ...
            if ($request->parent_menu_id != null) {
                ...
                if ($request->parent_menu_id != $request->old_parent) {
                    ...
                    if ($old_subs) {
                        foreach ($old_subs as $value) {
                            if ($value->no_urut > $request->old_serial_sub) {
                                DB::table('menus')
                                ->where('parent_menu_id', $request->old_parent)
                                ->where('no_urut', $value->no_urut)
                                ->update(['no_urut' => $value->no_urut - 1]);
                            }
                        }
                    }
                } else {
                    ...
                    if ($request->serial_sub_menu != $request->old_serial_sub) {
                        DB::table('menus')
                            ->where('parent_menu_id', $request->parent_menu_id)
                            ->where('no_urut', $request->serial_sub_menu)
                            ->update(['no_urut' => $request->old_serial_sub]);
                    }
                }
            } else {
                //jika menu
                ...
                if ($request->serial_menu != $request->old_serial_menu) {
                    DB::table('menus')
                        ->where('parent_menu_id', 0)
                        ->where('no_urut', $request->serial_menu)
                        ->update(['no_urut' => $request->old_serial_menu]);
                }
            }
